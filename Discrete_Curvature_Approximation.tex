...
A value of curvature can be calculated for each edge in the \textit{discretization} by considering the corresponding osculating circle on a given edge. The osculating circle here (circle, \Cref{fig:OsculatingCircle}) can be approximated by considering the circumscribed circle (circumcircle) \cite{casey1888} defined by the two end points of the edge, $P_0$ and $P_0$, and a point, P, between them in the \textit{curve} parameterization (\Cref{fig:CircumscribedCircle}) -- the radius of the circumcircle will be referred to as the discrete radius of curvature.
